...
{
    if (i == 0) return 0;
    else if(u_half > 0 && uhalf > 0) pui(p_half, phalf, u_half, uhalf) + ui(u_half, uhalf) * (p_half - phalf);
    else if(u_half < 0 && uhalf < 0) pui(p_half, phalf, u_half, uhalf) - ui(u_half, uhalf) * (p_half - phalf);
    else if((int)sgn(uhalf) != (int)sgn(u_half) && ui(u_half, uhalf) < 0) phalf * ui(u_half, uhalf);
    else p_half * ui(u_half, uhalf);
}
...
{
    if (i == N) return 0;
    else if(u_half > 0 && uhalf > 0) pui(p_half, phalf, u_half, uhalf) - ui(u_half, uhalf) * (phalf - p_half);
    else if(u_half < 0 && uhalf < 0) pui(p_half, phalf, u_half, uhalf) + ui(u_half, uhalf) * (phalf - p_half);
    else if((int)sgn(uhalf) != (int)sgn(u_half) && ui(u_half, uhalf) > 0) p_half * ui(u_half, uhalf);
    else phalf * ui(u_half, uhalf);
} */
...
void solver_in_time(double *u, double *phalf1, double *phalf, double *p_half, double *right_part, double *p0, double *f, double t_1, double h, int N, int M){
    /* memcpy(f, right_part, N);
    //setup_array(u, phalf1, phalf, p_half, f, p0, t_1, h, N);
    //setup_eq(u, phalf1, phalf, p_half, f, p0, t_1, 0, N);
    solver(p_half, phalf, phalf1, f, p0, N); */
    // решение записано в f, прошлая плотность записана в p0.
    // solver(p_half, phalf, phalf1, right_part, p0, N); // это функция от начальной плотности, которая известна
    // memcpy(f, right_part, N * sizeof(double));
    for (int j = 0; j < M; j++){
        // memcpy(f, right_part, sizeof(double) * N);
        /* print_array(f, N);
        print_array(right_part, N); */
        setup_array(u, phalf1, phalf, p_half, right_part, p0, t_1, /* h, */ N);
        // print_array(p0, N);
        print_matrix(phalf1, phalf, p_half, right_part, N);

        //////////print_matrix(phalf1, phalf, p_half, N);
        // setup_eq(u, phalf1, phalf, p_half, f, p_prev, t_1, j, N);
        // setup_eq(u, phalf1, phalf, p_half, right_part, p_prev, t_1, j, N);
        // до этого надо построить нужную матрицу
        //if (j == 0) solver(); // применить это к p0, в строке ниже был else
        // solver(p_half, phalf, phalf1, right_part, p_prev, N); // p_prev здесь нужно только для того, чтобы записать туда прошлую плотность. p_prev было нужно только для того, чтобы задать уравнения
        solver(p_half, phalf, phalf1, p0, N);
        ///////////print_array(p0, N);
        // print_array(p0, N);
        // применить это к p_prev
        // копировать решение в p_prev
    }
}
